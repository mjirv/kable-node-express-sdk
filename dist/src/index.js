...
class Kable {
    ...
    constructor(config) {
         ...
         * @returns
         */
        this.record = (clientId, data, transactionId, callback) => {
            if (this.debug) {
        ...
        this.enqueueEvent = (clientId, data, transactionId, callback) => {
            ...
            event["transactionId"] = transactionId || (0, uuid_1.v4)();
            event["data"] = data;
            const library = {};
            library["name"] = package_json_1.default.name;
            library["version"] = package_json_1.default.version;
        ...
        this.flushQueue = (callback) => {
            ...
            const items = this.queue.splice(0, this.maxQueueSize);
            const events = items.map((i) => i.event);
            const callbacks = items.map((i) => i.callback);
            const finish = (error) => {
            ...
            (0, axios_1.default)({
                ...
                .then(() => {
                console.debug(`[KABLE] Successfully sent ${events.length} events to Kable server`);
                finish();
            })
                .catch((error) => {
        ...
        const disableCache = Boolean(config.disableCache) || false; // for backward compatibility
        if (disableCache) {
            this.maxQueueSize = 1;
        }
        ...
        this.validCache = new node_cache_1.default({
            stdTTL: 30,
            maxKeys: 1000,
            checkperiod: 300,
        });
        this.invalidCache = new node_cache_1.default({
            stdTTL: 30,
            maxKeys: 1000,
            checkperiod: 300,
        });
        (0, axios_retry_1.default)(axios_1.default, {
            ...
            onRetry: (retryCount, error, requestConfig) => {
                var _a;
                if (this.debug) {
                    console.log(`[KABLE] Retrying failed event flush (${(_a = error.response) === null || _a === void 0 ? void 0 : _a.status}, retry ${retryCount})`);
                }
            },
        });
        (0, axios_1.default)({
